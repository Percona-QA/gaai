...
local function randit(gene)
  ...
  --innodb-buffer-pool-size(5242880,1073741824)       -- 5MB to 1GB     (Start: 5MB ) Gene:0
  --innodb-buffer-pool-chunk-size(1048576,268435456)  -- 1MB to 256MB   (Start: 1MB ) Gene:1
  ...
  --innodb-concurrency-tickets(1,5000)                -- 1 to 5000      (Start: 1   ) Gene:5  (May require smaller range)
  --innodb-flush-neighbors(0,1,2)                     -- 0 to 2         (Start: 2   ) Gene:6
  ...
  --innodb-change-buffering(none,inserts,deletes,changes,purges,all)    (Start: none) Gene:13 (mapped 0-5)

  if     gene == 0  then return math.random(5242880,1073741824)
  elseif gene == 1  then return math.random(1048576,268435456)
  ...
  elseif gene == 6  then return math.random(0,1,2)
